...
data = np.loadtxt("homo_eps0.5.txt", skiprows = 1)

exp_x = (data[:, 0] - 50.9)/10.7
gamma = 1.018
exp_F = -data[:, 1]/2/3.14/gamma/10.7
exp_err = data[:, 2]/2/3.14/gamma/10.7

plt.errorbar(exp_x + np.cos(0.0/180.0*np.pi), exp_F, exp_err, fmt = "o--", capsize = 2.0, fillstyle = 'none', color = 'r')

# Plot the macroscopically theoretical prediction of the curve
data2 = np.loadtxt("theoretical_curve.txt", skiprows = 1)


plt.plot(data2[:, 0]+ np.cos(0.0/180.0*np.pi), data2[:, 1], '-', color = "g")
