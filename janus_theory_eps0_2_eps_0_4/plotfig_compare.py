...
data = np.loadtxt("eps0.2.eps0.4.txt", skiprows = 1)

exp_x   = (data[:, 0] - inter_loc)/eff_rad
exp_F   = -data[:, 1]/2/3.14/gamma/eff_rad
exp_err = data[:, 2]/2/3.14/gamma/eff_rad

plt.errorbar(exp_x, exp_F, exp_err, fmt = "o--", capsize = 2.0, fillstyle = 'none', color = 'r', label = r"Janus $\epsilon = 0.4$")


data2 = np.loadtxt("homo_eps0.4.txt", skiprows = 1)
